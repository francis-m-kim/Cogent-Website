...
function submitHandler() {
  ...
  $('#submit').click(function() {
    ...
    var productDescription = $('#textarea').val();
    ...
    if (name && email && isEmail(email) && field && textarea) {
      document.getElementById('the-cogent-argument-form').submit();
    }
  })
  ...
  function sendEmail(name, email, field, phone, companyName, companyWebsite, productDescription) {
    console.log(name, email, field, phone, companyName, companyWebsite, productDescription);
    $.ajax({
      url: 'the-cogent-argument.php',
      type: 'POST',
      data: {
        name: name,
        email: email,
...
function cloudControl() {
  ...
  $('.clouds').each(function() {
    ...
    setTimeout(createNewCloud, 1000);
    function createNewCloud() {
      var newCloud = $('<img>').attr('src', path)
                               .addClass(classes)
                               .css('left', randomize(165,25) +"%");


      container.prepend(newCloud);
      // top = parseFloat(randomize(0,25))+ top;
      // newCloud.css('top', top+"%");
